<?php include "navs/nav_cuest_cereal.php" ?>
<section class="container-fluid bg-im3 padingtop padingbot">
    <div class="container">
        ...
        <?php if($cuestRespondidos == null){?>
            <?php include "cuestionarios/cuestCereal.php"?>
        <?php }
        else {?>
            <?php
            $cuestResp=$this->mis_funciones->limpiaOcho($cuestRespondidos);
            if(in_array($cuestionario,$cuestResp)){?>
                <div class="row">
                    <div class="col-md-6 col-md-offset-3">
                        ...
                        <a id='volver' class='btn  btn-cuest titulo4 center-block zoom' href='<?php echo base_url().'aplicacion/alimentos'?>'>Volver</a>
                    </div>
                </div>
            <?php }
            else {?>
...
<script>
    $(document).ready(function() {
        ...
        //var texto="<?php echo $preguntasCereal[0]->idpregunta?>";
        var preguntas=<?php echo json_encode($preguntasCereal,JSON_PRETTY_PRINT)?>;//arreglo de preguntas desde base de datos
        ...
        var avance=<?php echo $avance->avance_cuest_cereal?>;
        $(".inputcuest").on({
            click:function() {
                var i;
        ...
        $("#verificacuestionario").on({
            click:function(){
                ...
                if(respondido){
                    ...
                    guardaCuestionario('<?php echo base_url()."aplicacion/guardaCuestCer"?>',cuestionario);
                    avance++;
                    actualizaAvance('<?php echo base_url()."aplicacion/actualizaAvance"?>',avance,"cuestCereal");
                    $("#guardar").append("<a id='volver' class='btn  btn-cuest titulo4 center-block zoom' href='<?php echo base_url().'aplicacion/cereales#section2'?>'>Volver</a>");
